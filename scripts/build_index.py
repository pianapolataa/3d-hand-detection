"""
build_index.py — Precompute backbone embeddings for the FreiHAND eval set.

Runs every image through the trained model, extracts the 512-dim ResNet18
backbone feature (via a forward hook), L2-normalises them for cosine
similarity, and saves the result alongside predicted joints / vertices so
retrieve.py can display full point clouds without re-running inference.

Usage (from scripts/):
    python build_index.py
    python build_index.py --npz ../data/eval_data_778_verts.npz \
                          --model checkpoints/best_point_model_newest_12560.pth \
                          --num-verts 778 --out ../data/retrieval_index_778_verts.npz
"""
